...
function TileSortable({ tile, isPlaceholder }: TileSortableProps) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className={`tile ${isPlaceholder ? "placeholder-tile" : ""}`}>
      { !isPlaceholder &&
        <>
...
export function WordRack({
  ...
  const { setNodeRef: setRackNodeRef, isOver: isRackOver } = useDroppable({ id: `rack-${rackIndex}-end` });

  return (
    <div className="word-rack-row">
        <div className={`word-rack ${rackScore.baseScore > 0 ? "valid" : "invalid"}`}>
          {tiles.map(tile => (
            <TileSortable key={tile.id} tile={tile} isPlaceholder={tile.id === activeTileId} />
          ))}
          <div
            ref={setRackNodeRef}
