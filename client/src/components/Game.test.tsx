...
describe("Game Component - Integration Tests", () => {
	...
	describe("Other", () => {
		...
		it("should not display the submit button for a partially solved board", async () => {
			render(<Game puzzle={MOCK_PARTIALLY_SOLVED_PUZZLE} gameRules={MOCK_RULES} initialHistory={null} />)
		...
		it("should prevent dragging a tile to a full rack", async () => {
			...
			render(<Game puzzle={MOCK_PUZZLE} gameRules={MOCK_RULES} initialHistory={null} maxTiles={5} />)
			await user.click(screen.getByRole("button", { name: /start game/i }))
